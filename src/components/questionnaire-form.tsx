...
export function QuestionnaireForm() {
  ...
  // Pre-fill the accessibility fields from the saved onboarding profile.
  // The form is uncontrolled, so we read the profile after mount and remount the
  // affected fields via `hydrated` so their defaultValue/defaultChecked refresh.
  const [profile, setProfile] = useState(defaultAccessibilityProfile);
  ...
  const mobilityDefault = deriveMobilityLevel(profile);
  const checkboxDefaults: Record<string, boolean> = {
    needElevatorAccess: profile.avoidStairs,
    needWheelchairAccessibleTransportation: mobilityDefault === "wheelchair",
    avoidStairs: profile.avoidStairs,
    needFrequentRestBreaks: profile.frequentRestBreaks,
    accessibleBathroomRequired: profile.accessibleRestroomRequired,
  };
            ...
            <section>
              ...
              <div className="grid gap-5">
                ...
                <label className="block font-semibold text-slate-800">
                  רמת ניידות
                  <select
                    key={`mobilityLevel-${hydrated}`}
                    className={inputClass}
                    name="mobilityLevel"
                    defaultValue={mobilityDefault}
                    disabled={isLoading}
                  >
                ...
                <label className="block font-semibold text-slate-800">
                  ...
                  <input
                    key={`maxWalkingTimePerDay-${hydrated}`}
                    className={inputClass}
                    name="maxWalkingTimePerDay"
                    type="number"
                    min="0"
                    max="240"
                    defaultValue={profile.maxWalkingMinutes}
                    disabled={isLoading}
                  />
                </label>
                <label className="block font-semibold text-slate-800">
                  ...
                  {[
                    ...
                    <label key={name} className="flex items-center gap-3 text-base font-semibold text-slate-800">
                      <input
                        ...
                        defaultChecked={checkboxDefaults[name]}
                        disabled={isLoading}
                      />
                      {label}
                    </label>
            ...
            <section>
              ...
              <div className="grid gap-5">
                <label className="block font-semibold text-slate-800">
                  העדפות תזונה
                  <select
                    key={`dietaryPreference-${hydrated}`}
                    className={inputClass}
                    name="dietaryPreference"
                    defaultValue={profile.dietary}
                    disabled={isLoading}
                  >
